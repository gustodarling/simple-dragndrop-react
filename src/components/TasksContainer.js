...
class TasksContainer extends Component {
  ...
  drop(e) {
    ...
    e.preventDefault();
    let elem = e.target.nodeName == 'LI' ? e.target.parentElement.parentElement : e.target.parentElement
    let cls =  elem.getAttribute('class') != null?elem.getAttribute('class').split(' ')[1]:''
    switch(cls) {
      case 'progress':
        this.props.dragDrop(this.state.currentTaskDragged,'inProgress')
        break
      case 'todo':
        this.props.dragDrop(this.state.currentTaskDragged,'todo')
        break
      case 'done':
        this.props.dragDrop(this.state.currentTaskDragged,'done')
        break
    }
    return false
  }
